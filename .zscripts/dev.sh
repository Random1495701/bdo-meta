...
cleanup() {
        # Do NOT kill the dev server on exit — it needs to stay alive for z.ai preview.
        # The server is disowned (see below) so it survives script exit.
        echo "[dev.sh] Script exiting. Dev server (PID: ${DEV_PID:-unknown}) left running."
}

# Only cleanup on INT/TERM (Ctrl+C / kill signal), NOT on normal EXIT
