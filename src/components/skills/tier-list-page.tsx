...
import {
  Swords, Zap, Shield, ShieldHalf, Grab, Target, Gauge,
  RotateCcw, SlidersHorizontal, Table2, LayoutList, Crown,
  TrendingUp, Info, X, ChevronDown, Search,
} from 'lucide-react'
...
const PRESETS: Record<string, { label: string; weights: Weights; description: string }> = {
  ...
  control: {
    ...
    weights: { ...ZERO_WEIGHTS, pvpCcSkillCount: 100, ccChainPotential: 80, grabCount: 90 } as Weights,
  },
  defense: {
    label: 'Defense',
    description: 'Prioritize protection and SA damage reduction.',
    weights: { ...ZERO_WEIGHTS, superArmorCount: 80, forwardGuardCount: 70, iFrameCount: 90, coreSaCount: 60, coreFgCount: 50, protectedCoverage: 75, saDr: 100 } as Weights,
  },
  burst: {
    label: 'Burst (DPS)',
    description: 'Fast, hard-hitting skills with some CC.',
    weights: { ...ZERO_WEIGHTS, dpsEstimate: 100, avgPvpDamage: 70, pvpCcSkillCount: 40, ccChainPotential: 30 } as Weights,
  },
  bruiser: {
    label: 'Bruiser',
    description: 'Balance damage, protection, and grabs.',
    weights: { ...ZERO_WEIGHTS, avgPvpDamage: 50, superArmorCount: 60, forwardGuardCount: 50, grabCount: 70, pvpCcSkillCount: 50, saDr: 60, protectedCoverage: 50 } as Weights,
  },
}
...
function loadWeights(): Weights {
  ...
  try {
    ...
    if (raw) {
      ...
      // Merge with zero weights to ensure all keys exist
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* Mini param bars (top 3 weighted) */}
...
function scoreColor(pct: number): string {
  ...
  if (pct >= 20) return '#a78bfa' // purple
  return '#f87171' // red
}
