...
export function SessionTrackerPage() {
  ...
  // 3-step paste-JSON flow state
  const [screenshotUrl, setScreenshotUrl] = React.useState<string | null>(null)
  const [parsePrompt, setParsePrompt] = React.useState<string | null>(null)
  const [suggestedServices, setSuggestedServices] = React.useState<{ name: string; url: string }[]>([])
  const [pasteJsonText, setPasteJsonText] = React.useState('')
  const [pasteError, setPasteError] = React.useState<string | null>(null)
  const [pasteSuccess, setPasteSuccess] = React.useState<string | null>(null)
  ...
  // Step 1: Upload screenshot — backend saves the image and returns the URL +
  // a JSON-extraction prompt + suggested AI services. The actual vision parsing
  // happens externally (ChatGPT/Gemini/Claude/Copilot); the user pastes the
  // returned JSON back in step 3.
  const handleScreenshotUpload = async (file: File) => {
    ...
    try {
      ...
      setParsePrompt(data.parsePrompt)
  ...
  // Copy the extraction prompt to the clipboard for easy pasting into external AI
  const handleCopyPrompt = async () => {
    if (!parsePrompt) return
    try {
      await navigator.clipboard.writeText(parsePrompt)
      setPasteSuccess('Prompt copied — paste it + your screenshot into an AI below.')
      setTimeout(() => setPasteSuccess(null), 3000)
    } catch {
      // Fallback: select the textarea contents
      promptTextareaRef.current?.select()
      document.execCommand('copy')
      setPasteSuccess('Prompt copied (fallback).')
      setTimeout(() => setPasteSuccess(null), 3000)
    }
  }

  // Step 3: Parse the JSON pasted back from an external AI and create a session
  const handlePasteJsonSubmit = async () => {
    setPasteError(null)
    setPasteSuccess(null)
    if (!pasteJsonText.trim()) {
      setPasteError('Paste the JSON returned by the AI first.')
      return
    }
    if (!playerTag.trim()) {
      setPasteError('Enter your player name first.')
      return
    }
    ...
    try {
      ...
      const jsonMatch = pasteJsonText.match(/\{[\s\S]*\}/)
      p = JSON.parse(jsonMatch ? jsonMatch[0] : pasteJsonText)
    } catch (e) {
    ...
    try {
      ...
      setPasteSuccess('Session created from pasted JSON.')
      // Reset paste flow
      setPasteJsonText('')
      setScreenshotUrl(null)
      setParsePrompt(null)
      ...
      setPasteError(`Failed to create session: ${(e as Error).message}`)
    }
  }
          ...
          <div className="rounded-sm border-2 border-amber-800/40 bg-bdo-leather-dark/30 p-4">
            <div className="mb-3 flex flex-wrap items-center gap-3">
              ...
              >
                ...
                {uploading ? 'Saving...' : 'Upload Screenshot'}
              </Button>
              <input
                ref={fileInputRef}
                type="file"
            ...
            {/* Paste-success toast */}
            {pasteSuccess && (
              <div className="flex items-center gap-2 rounded-sm border border-emerald-700/40 bg-emerald-900/10 p-2 text-xs text-emerald-300">
                <CheckCircle2 className="size-3.5" />
                {pasteSuccess}
              </div>
            )}

            {/* 3-step paste-JSON flow */}
            {screenshotUrl && parsePrompt && (
              <div className="mt-3 space-y-3 rounded-sm border border-amber-800/30 bg-bdo-ink/40 p-3">
                ...
                <div className="flex items-center gap-2 text-xs text-amber-200/70">
                  ...
                  >
                    <ImageIcon className="size-3" /> {screenshotUrl}
                  </a>
                </div>

                {/* Step 2: copyable prompt + service buttons */}
                <div>
                  <div className="mb-1 flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-amber-300/60">
                      Step 2 — paste this prompt + screenshot into an AI
                    </span>
                    <Button
                      onClick={handleCopyPrompt}
                      size="sm"
                      variant="outline"
                      className="h-6 border-amber-800/40 px-2 text-[10px] text-amber-300/70 hover:text-amber-200"
                    >
                      Copy Prompt
                    </Button>
                  </div>
                  <textarea
                    ...
                    readOnly
                    value={parsePrompt}
                    className="h-32 w-full resize-y rounded-sm border border-amber-800/40 bg-bdo-ink/60 p-2 font-mono text-[10px] leading-snug text-amber-100/80"
                  />
                  <div className="mt-2 flex flex-wrap gap-2">
                    {suggestedServices.map((svc) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 rounded-sm border border-amber-800/40 bg-bdo-leather-dark px-2.5 py-1 text-[11px] text-amber-300/80 transition-colors hover:border-amber-500/60 hover:text-amber-200"
                      >
                        <ImageIcon className="size-3" /> {svc.name}
                      </a>
                    ))}
                  </div>
                </div>
                ...
                {/* Step 3: paste JSON */}
                <div>
                  <div className="mb-1 text-[10px] font-bold uppercase tracking-wider text-amber-300/60">
                    Step 3 — paste the JSON returned by the AI
                  </div>
                  <textarea
                    value={pasteJsonText}
                    onChange={(e) => setPasteJsonText(e.target.value)}
                    placeholder='{"sessionType":"aos","result":"victory","playerStats":{...}, ...}'
                    className="h-40 w-full resize-y rounded-sm border border-amber-800/40 bg-bdo-ink/60 p-2 font-mono text-[10px] leading-snug text-amber-100/80 placeholder:text-amber-300/30"
                  />
                  {pasteError && (
                    <div className="mt-1 flex items-center gap-1.5 text-[10px] text-red-300">
                      <AlertCircle className="size-3" /> {pasteError}
                    </div>
                  )}
                  <div className="mt-2 flex gap-2">
                    <Button
                      onClick={handlePasteJsonSubmit}
                      disabled={createMutation.isPending || !pasteJsonText.trim()}
                      className="bdo-btn"
                      size="sm"
                    >
